      ...
      {steps.map((step, index) => (
        <div key={step.label} className="flex items-center flex-1 last:flex-none">
          <div className="flex flex-col items-center relative z-10">
            ...
            <span className={`absolute -bottom-5 left-1/2 -translate-x-1/2 text-[9px] font-bold uppercase whitespace-nowrap ${step.active ? 'text-secondary' : 'text-outline'}`}>
              {step.label}
            </span>
          </div>
          {index < steps.length - 1 && (
            <div className={`flex-1 h-[2px] mx-1 sm:mx-2 min-w-[20px] ${steps[index + 1].active ? 'bg-secondary' : 'bg-surface-container'}`} />
          )}
        </div>
      ))}
    </div>
